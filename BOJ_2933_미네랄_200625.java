...
public class Main {
    ...
    private static void down(int targetCluster) {
        ...
        int dist = R; //INF
        for (int j = 0; j < C; j++) {
            int tc = R - 1; //lowest height of target cluster
            while (tc >= 0 && cluster[tc][j] != targetCluster) tc--;
            if (tc < 0) continue; //이 줄에는 클러스터가 업서요

            int tmp = tc + 1; //highest height of other clusters or ground
            while (tmp < R && cave[tmp][j] == '.') tmp++;

            if (dist > tmp - tc - 1) dist = tmp - tc - 1;
        }
